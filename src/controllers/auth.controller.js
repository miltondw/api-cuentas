    ...
    if (rol === 'admin') {
      ...
      if (!jwt2 || jwt2 !== process.env.JWT_SECRET_2) {
        return res.status(403).json({ 
          error: "Código de autorización inválido para crear cuenta de administrador" 
        });
      }
    }
...
const loginUsuario = async (req, res) => {
  ...
  try {
    ...
    if (lockStatus.isLocked) {
      const waitMinutes = Math.ceil((lockStatus.lockedUntil - new Date()) / (1000 * 60));
    ...
    // 8. Responder con información del usuario (sin datos sensibles)
    res.status(200).json({
